...
export function initGlobe(canvas, DATA, TOPO, options = {}){
  // A click returns a small set of country facts.
  ...
  // The narrative payload keeps country names on each study. It does not ship graph edges.
  const byName = {}
  const detail = {}
  for (const study of DATA.nodes.filter(node => node.type === 'study')){
    const isReview = (study.studyTypes || []).includes('EVIDENCE_SYNTHESIS')
    for (const rawName of new Set(study.countries || [])){
      const key = NAME_FIX[rawName] || rawName
      byName[key] = (byName[key] || 0) + 1
      const row = detail[key] || (detail[key] = { studies:0, instruments:new Map() })
      row.studies += 1
      if (!isReview){
        for (const instrument of new Set(study.instruments || []))
          row.instruments.set(instrument, (row.instruments.get(instrument) || 0) + 1)
      }
    }
  }
  const counts = {}
  ...
  let selected = false               // a selected country holds still while its card is read
  ...
  function frame(){
    ...
    if (!drag && !selected && now > autoResume) rot[0] -= dt * SPIN
    pin += (pinTo - pin) * Math.min(1, dt * 7)
    mx += (tmx - mx) * 0.05; my += (tmy - my) * 0.05
    proj.rotate(rot)
    ctx.clearRect(0, 0, W, H)

    /* Below 640px the globe has its own boxed-off strip rather than sharing the fold with the
       words, so it is centred there instead of parked at the right edge like the desktop composition. */
    const mobile = W <= 640
    // lower in the frame, and drifting — the parallax you asked for
    const cx = W * (mobile ? 0.5 : 0.70) + mx * (mobile ? 18 : 26)
    /* The globe RISES as the page scrolls, it does not sink.  */
    const cy = H * ((mobile ? 0.5 : 0.58) - park * 0.08) + my * 16
  ...
  function size(){
    ...
    // Its own strip is short and wide, so it gets more of the width and less of the height to work with.
    const R = W <= 640 ? Math.min(H * 0.42, W * 0.34) : Math.min(H * 0.40, W * 0.29)
  ...
  const onCanvasClick = ev => {
    ...
    if (!hover){ selected = false; onSelect(null); return }
    const name = hover.properties.name
    const row = detail[name]
    const topInstrument = row
      ? [...row.instruments].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0]?.[0] || null
      : null
    selected = true
    onSelect({
      name,
      studies:row?.studies || counts[name] || 0,
      topInstrument,
  ...
  return {
    ...
    clearSelection(){ selected = false; onSelect(null) },
    setActive(next){
      active = Boolean(next)
      if (!active){
