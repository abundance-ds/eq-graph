...
function citedWork(studies, width, height, data, coauthors, cites){
  ...
  /* One pitch for the whole chart, set by the longest row, so a dot means the same thing on every line. */
...
function coauthorNetwork(studies, width, height, data, coauthors){
  ...
  const at = new Map()
  people.forEach((p, i) => {
    // a phyllotaxis spiral, so the start is spread rather than a ring, and deterministic so the same data always settles the same way
  ...
  const marks = nodes.map((n, i) => {
    ...
    // The count goes inside wherever the circle can hold it legibly. 
    const count = n.r >= 11
      ...
      : ''
    // Biggest arrive first, so the shape of the field is established before the detail fills in. 
  ...
  const smallest = Math.min(...people.map(p => p.paper_count))
      /* Each item declares its width and the row advances by it.  */
  const ky = 20
  const CHAR = 6.6
  let kx = 6
  const bits = []
...
export function createStoryCharts(data, root, coauthors = null, cites = null){
  ...
  function pick(scene, id){
    ...
    reform(scene, id)
      ...
      /* Hover is temporary and leaves nothing behind.  */
  let held = null

  /* The sheen is one element moved onto whichever circle is under the pointer, rather than one per node.  */
  ...
  /* Same rule as the country card: a kept selection is a temporary state, and the chart it belongs to slides away as soon as the reader scrolls. */
  ...
  function show(from = null, to = null, progress = 0, cross = 0){
    for (const [id, scene] of scenes){
      ...
      // Hit-testing follows what you can see. 
