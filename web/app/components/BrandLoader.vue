...
/* Each dot is placed by rotating it out to the rim, so the ring is one transform per dot rather than ten hand-written positions. */
.bl-ring i {
  position: absolute;
  inset: 0;
  margin: auto;
  ...
  /* Negative delays start the cycle already in progress, so the ring is mid-rotation on the first frame instead of switching on all at once. */
...
/* Movement is the whole point of this element, so with motion turned down it holds still and states itself rather than disappearing. */
