...
- The project-first pipeline has **305 distinct works** linked at accepted confidence and **287 full texts** on disk.
  The 220 JATS XML ones are converted to Markdown in [`corpus/`](corpus/README.md);
  `to_markdown.py` now converts the 67 PDFs too, via poppler rather than pandoc, but only the 7 in the [ontology pilot](docs/ontology-pilot/README.md) have been run so far.
