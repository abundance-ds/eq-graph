...
def build(source: Path, output: Path) -> dict[str, int]:
    ...
    try:
        ...
        use_rows = connection.execute(
            """
            SELECT use_id, study_id, use_type,
                   COALESCE(canonical_label, source_label), source_label,
                   context, function, analytic_role, registry_id, details_json
        ...
        connection.execute(
            ...
            SELECT u.use_id, u.study_id, COALESCE(u.canonical_label, u.source_label),
                   u.context, u.function, u.analytic_role
            FROM scientific_uses AS u WHERE u.use_type='Product'
            """
        )
        connection.execute(
            """
