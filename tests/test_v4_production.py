...
class ProductionRecordTests(unittest.TestCase):
    ...
    def test_typed_product_alias_can_have_separate_scoring_meaning(self) -> None:
        rows = [
            {
                "registry_id": "product:general",
                "entity_type": "Product",
                "canonical_label": "General product",
                "aliases": "",
            },
            {
                "registry_id": "product:scoring",
                "entity_type": "Product",
                "canonical_label": "Scoring product",
                "aliases": "",
            },
        ]
        aliases = [
        ...
        _, lookup = registry_lookup(rows, aliases)
        self.assertEqual(
            lookup[("Product", "shared source wording")], {"product:general"}
        )
        self.assertEqual(
            lookup[("Scoring", "shared source wording")], {"product:scoring"}
        )

    def test_source_scoped_identity_survives_a_record_id_change(self) -> None:
        identity = {
            "scope": "OLD-P001",
            "source_identifier": "10.1000/Example",
        }
        self.assertTrue(identity_in_scope(identity, "NEW-P001", "10.1000/example"))
        self.assertFalse(identity_in_scope(identity, "NEW-P002", "10.1000/other"))
