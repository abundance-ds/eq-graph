...
1. **Ontology requirements** (competency-question-driven design): the Neo4j schema
   ...
   is a schema gap.
2. **Evaluation benchmark**: once populated, each good question is scored
   *correct / partial / wrong / unanswerable*, and each bad question is scored on
   whether the system responds with the **expected behavior** (refuse, caveat, or
   correct the premise) instead of hallucinating an answer.

**How the set was built.** Personas × capability matrix: six user personas
(foundation staff/board, grant reviewers, active EQ researchers & members, PhD
students/newcomers, meta-researchers, impact/communications), crossed with query
capabilities (lookup, list, aggregation, ranking, temporal trend, graph traversal,
network analysis, similarity, negation/gap-finding, provenance) and output modalities
(text, number, list of studies, links, table, visualization, profile).

**Column legend**
- **Output**: text · number · list · links · table · viz · profile (mixed)
- **Needs**: `core` = projects+publications+authors+links · `extract` =
  extractions.json entities (instrument/method/country/condition/sample/finding) ·
  `cites` = citation edges · `member` = membership data · `budget` = budget/status
  fields · `sim` = text-similarity/embeddings · `fulltext` = harvested full text ·
  `class` = relevance-classification labels (C1/C2, F1)
- **Pilot**: ✓ answerable with the proposed pilot load (all 1,024 projects +
  318 accepted links + their works/authors + extractions for accepted projects) ·
  ✗ needs the full graph or later enrichment
...
Each has a category and an **expected behavior**: `refuse` (say the graph does not
hold this), `caveat` (answer the answerable part, state the boundary), `premise`
(correct the false premise), `no-judgment` (decline normative/subjective verdicts,
offer the factual substrate). Hallucinating a fluent answer = test failure.
...
- **Personas**: foundation/board 15 · reviewers 13 · researchers/members 24 ·
  PhD/newcomers 15 · meta-researchers 20 · impact/provenance 13.
- **Output modalities** (approx.): lists/links 45 · numbers 20 · viz 22 ·
  tables 15 · text/profiles 8 (some questions count twice).
- **Capabilities exercised**: lookup, aggregation, ranking, temporal trends,
  multi-hop traversal, network analytics, similarity search (Q18/Q20/Q28),
  negation/gap-finding (Q9, Q15, Q23, Q43, Q63), provenance (Q87, Q98, Q100),
  data-quality introspection (Q81, Q82).
- **Pilot-answerable**: 55 of 100 with the proposed pilot load; the ✗ questions
  mostly need the full relevance-classified corpus (`class`), citation edges at
  scale, affiliation→country resolution, or embeddings — each ✗ tag is therefore
  also a roadmap item.
