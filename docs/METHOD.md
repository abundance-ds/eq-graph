...
> **Historical design document.** This 2026-07-30 draft is not the governing
> screening method. Use [`../protocol-2.0.md`](../protocol-2.0.md) and
> [`METHOD_SIMPLE.md`](METHOD_SIMPLE.md). The draft remains here as design history.

Status: **draft for agreement** (Paul ↔ Kazik), 2026-07-30. Items marked **[decide]**
need explicit sign-off; everything else is proposed as default.
...
Kazik's pipeline come from `../FRIEND_REPO_EMPIRICAL_FINDINGS.md` (assessed at commit
`68ebeab`, 2026-07-29).
...
queries. Target store: Neo4j (Kazik), fed from replayable pipeline artefacts.

## 2. Study design

Two pipelines were built independently and are now merged:

| | Pipeline A (this repo, Paul) | Pipeline B (`shoulders-ai/eq-graph`, Kazik) |
|---|---|---|
...
| Source | Role | Access |
|---|---|---|
| EuroQol grant export, 1,024 projects | Canonical project table (id, title, abstract, PI, WG, **status, start/end year, budget**) | public export, downloaded 2026-07-28 (Kazik's repo) |
...
**[decide]** Adopt the 1,024-row export as the canonical project table. It strictly
...
> borderline C1/C3 cases go to the review queue. **[decide]** — this is exactly the
> rule from Paul's framing; the only judgment call is C3-default-exclude.
...
**[decide]** Graph membership: publication nodes only for the *included* set; the full
...
Merge both matchers into one **typed-evidence** model (Kazik's framework, absorbing
Paul's signals). Each (paper, project) pair carries a list of evidence items; the pair
score is the **max** individual weight (no summing). Proposed weights **[decide]**:
...
**Funnel 3 — Publication identification (union, deduped by DOI → PMID → OpenAlex id)**
...
1. **Deterministic rules.** F1 evidence or an accepted link → include. Title/abstract
   matching EQ-instrument terms (`EQ-5D`, `EuroQol`, `EQ-VT`, `EQ-HWB`, `EQ-TIPS`,
   value set/valuation/crosswalk/mapping/norms + instrument mention) → candidate band.
   No signal → auto-exclude, with a **100-paper random audit** of the excluded pool to
...
2. **LLM classification** of the candidate band (title + abstract) into
   ...
   Calibration first: Paul and Kazik independently hand-label the same **200 papers**
...
Proposed pilot population **[decide]**: all 1,024 project nodes (cheap, enables
...
- **Abstract availability:** ~36% of A's corpus lacks abstracts → content
