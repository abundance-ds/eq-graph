...
An EQ instrument as the main or only outcome does not by itself make a study
`direct_eq`. Use `application_only` when the research question concerns health,
treatment, risk factors, inequalities, or a service or program, and EQ supplies
an outcome. Use `direct_eq` when the research question concerns the EQ
instrument, measurement process, valuation, value set, mapping, reference
norms, or implementation as a measurement process.
...
End each substantive bullet with `**Source:**` and the source section, table,
figure, or paragraph. Do not reconstruct bibliographic metadata that the
structured input already supplies. Do not invent a fact to fill a heading.
