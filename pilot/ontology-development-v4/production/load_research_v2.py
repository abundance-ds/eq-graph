...
def load_registry(
    ...
    connection.executemany(
        "INSERT INTO registry_alias(registry_id, alias) VALUES (?, ?)",
        [(row["registry_id"], row["alias"]) for row in read_tsv(aliases_path)],
    )
...
def load_publication(
    ...
    connection.execute(
        ...
        INSERT INTO source_record(
            record_id, article_path, article_sha256, article_bytes, xml_path,
            xml_sha256, xml_bytes, extraction_record_path,
        ...
        (
            ...
            manifest_row["xml_path"],
            manifest_row["xml_sha256"],
            int(manifest_row["xml_bytes"]),
    ...
    for author in metadata["authors"]:
        ...
        orcid = (author.get("orcid") or "").strip().lower() or None
        person_id = f"orcid:{orcid}" if orcid else f"author:{publication_id}:{order}"
        connection.execute(
...
def main() -> None:
    ...
    try:
        ...
        for manifest_row in manifest_rows:
            ...
            errors, _ = validate_record(record, manifest_row["record_id"], args.registry.resolve())
            if errors:
