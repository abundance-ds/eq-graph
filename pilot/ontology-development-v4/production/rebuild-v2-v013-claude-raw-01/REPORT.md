...
- Records: 209/209.
- Studies: 207.
- Unmapped registry labels: 4044.
- Ontology gaps: 194.
- Source conflicts: 117.
- Deterministically valid records: 168/209.
...
- `UNMODELED_ASPECT` `scoring_product_application`: Disability weights (WHO 2013, GBD 2013, Murray 1994) are applied to prevalent cases of vision impairment to derive DALYs, rather than to instrument responses, so the application of this reusable weight set does not fit ScoringUse.
...
- `UNCERTAIN_MAPPING` `comparison_structure`: The planned comparison is between two modes of administration (face-to-face vs videoconference) of the same cTTO method, which fits both BETWEEN_CONTEXT ('modes') and BETWEEN_METHOD.
...
- `NOT_REPORTED` `software used for conditional logit estimation and bootstrap`: Methods report conditional logit estimation and cluster bootstrap with percentile method but name no software; the discussion notes that no open-source software is available for kaizen-task DCEs.
...
- `UNCERTAIN_MAPPING` `comparison_structure`: The planned comparison is between nested prediction models (Base vs EQ-5D-5L-enhanced vs VR-12-enhanced) rather than between instruments per se, although the two PROMs distinguish the enhanced models.
- `UNMODELED_ASPECT` `function`: The single FACIT-COST item FT12 was used as an external anchor for test-retest and responsiveness analyses and was excluded from the total score, but no use type represents a single item within an instrument acting as an anchor.
...
- `UNMODELED_ASPECT` `scoring`: The paper states that the EQ-5D-5L can be scored using national value sets for QALY calculation but that this approach was not used; level sum scores were computed instead
- `UNCERTAIN_MAPPING` `instrument_function`: The open-ended questionnaire collects qualitative reports of problems and solutions from interviewers; no instrument function describes qualitative data collection, so OUTCOME_MEASURE was used as the closest fit.
...
- `UNCERTAIN_MAPPING` `origin`: Methods state the study used self-reported cross-sectional data from an online survey sent in December 2018 by Q-uestion and Q-support with a citation to a previously published ten-year evaluation study [6], while ethics approval and measures are described as belonging to this study.
...
- `UNMAPPED_VALUE` `software_function`: Google reCAPTCHA v3 was implemented across four interactions to flag bot responses; its role is quality control of collected data, but the software_function vocabulary has no quality-control value.
...
- `NOT_REPORTED` `pilot interview count`: The source states only that each interviewer completed at least two sets of 5 pilot interviews, without a total pilot interview count.
...
- `UNMODELED_ASPECT` `comparison_structure`: The study combines a planned comparison of EQ-5D-5L, EQ-HWB, EQ-HWB-9 and BIT (between-instrument), known-groups comparisons by sweating and weather adaptation (between-group), and within-person score change across waves (within-person), but only one comparison value can be recorded per axis.
...
- `UNCERTAIN_MAPPING` `mean age of the SUD patient sample`: Methods report mean age 23.9 years (SD = 3) while Results report mean 23.8 years (SD 3.2) for the same 178 patients.
...
- `UNCERTAIN_MAPPING` `function`: The PTO method is itself an object of investigation (how participants interpret PTO tasks and whether the questions capture relative weights), but the method_function vocabulary offers no value for a method examined as a study object; PREFERENCE_ELICITATION was retained with CURRENT_STUDY_OBJECT context.
...
- `UNMAPPED_VALUE` `product_function`: The Canadian time trade-off-derived EQ-5D-5L value set is cited only as an example scoring algorithm in the narrative; it is neither an analysis object, comparator, nor evidence-synthesis target.
...
- `NOT_REPORTED` `size`: Known-group analyses in Table 6 report group totals summing to 112 rather than the 117 completers, without an explanation of the difference.
...
- `UNMAPPED_VALUE` `product_function`: The Van Hout et al. crosswalk algorithm was both applied to derive crosswalk value sets and examined as the object of the comparison; the product_function vocabulary has no value for an existing product applied as a derivation tool.
...
- `UNMAPPED_VALUE` `product_function`: The published Dutch 3L and 5L tariffs were used as rescaling anchors for the DC values, not strictly as analysis objects or comparators.
...
- `NOT_REPORTED` `methods_and_results`: 'Given a planned experimental arm of our research... (the methods and results of the experimental substudy will be reported elsewhere)' and 'After excluding interviews with experimental TTO blocks (the results of the study will be reported elsewhere)'.
...
- `UNMAPPED_VALUE` `outcome_family`: The reported outcomes are stakeholders' normative views and methodological recommendations on youth valuation, which fit no exact outcome family; CONCEPTUAL_CLASSIFICATION was the closest available value.
...
- `UNCERTAIN_MAPPING` `publication_form`: The article is labelled a 'Current Opinion' by the journal but its principal output is a developed taxonomy with evidence requirements and solutions, fitting either OPINION_ARTICLE or CONCEPTUAL_ARTICLE.
...
- `UNMAPPED_VALUE` `comparison_structure`: The review's principal comparison is between two publication time periods (1999–2010 vs 2011–2019) as well as between countries; no controlled comparison_structure value expresses a between-time-period comparison.
...
- `UNMAPPED_VALUE` `comparison_structure`: The study combines within-person pre/post responsiveness analysis with between-group known-groups (discriminant) validity comparisons by tumor stage, ECOG status, age and treatment modality.
...
- Number of cTTO observations clustered at − 1
- Availability of general population data at 1-year follow-up
...
- Reported ceiling values for the tiredness (TI) bolt-on
...
- Counts of studies versus HSUs using unpublished empirical data
...
- Proportion of content validity participants with a chronic condition
...
- `V2-P012` `InstrumentUse`: Chichewa (Malawi) PedsQL™ 4.0 GCS child self-report
- `V2-P012` `InstrumentUse`: Chichewa (Malawi) PedsQL™ 4.0 GCS teen self-report
- `V2-P012` `InstrumentUse`: EQ-5D-Y-3L (Chichewa version)
- `V2-P012` `InstrumentUse`: EQ-5D-Y-5L (Chichewa version)
...
- `V2-P012` `MethodUse`: Multitrait-multimethod (MTMM) approach modified by Henseler et al.
...
- `V2-P016` `InstrumentUse`: EQ-5D-Y-3L
- `V2-P016` `InstrumentUse`: self-reported EQ-5D-Y-3L instrument (warm-up task)
...
- `V2-P030` `ProductUse`: Individualized online patient decision aid for total knee arthroplasty (with summary report)
...
- `V2-P032` `MethodUse`: Pearson correlations of individual-level difference scores
...
- `V2-P038` `MethodUse`: pilot tests of the Shiny interviewing software
...
- `V2-P046` `InstrumentUse`: Split anxiety and depression subdimensions (EQ-4D-5L + anxiety + depression)
...
- `V2-P066` `ProtocolUse`: Janssen, Szende & Ramos-Goñi, Chapter 2: Data and Methods (Self-Reported Population Health)
...
- `V2-P073` `ScoringUse`: UK EQ-5D-3L tariff (Dolan, Modeling valuations for EuroQol health states)
- `V2-P073` `MethodUse`: Enrolment of consecutive patients in the Youth Addiction Treatment Evaluation Project residential treatment unit
...
- `V2-P077` `InstrumentUse`: EQ-5D-3L (adult version wording for usual activities examples)
...
- `V2-P080` `ProductUse`: Canadian EQ-5D-5L value set (time trade-off-derived scoring from the general Canadian population)
...
- `V2-P088` `ProductUse`: English EQ-5D-5L value set
...
- `V2-P091` `InstrumentUse`: PedsQL™ (Portuguese-Brazilian version)
...
- `V2-P106` `InstrumentUse`: EQ-5D-5L + Rs descriptive responses
- `V2-P106` `InstrumentUse`: Self-reported primary cause of breathlessness (multiple-choice question)
- `V2-P106` `InstrumentUse`: EQ-5D-5L with respiratory bolt-on dimensions and EQ-VAS as administered in the online breathlessness survey
...
- `V2-P119` `ScoringUse`: EQ-5D-Y-3L utility scoring algorithm (Ramos-Goñi et al. 2022 EQ-5D-Y value set) [25]
...
- `V2-P131` `InstrumentUse`: four QoL measures (four preference-weighted quality-of-life measures)
...
- `V2-P142` `ProductUse`: US EQ-5D-5L value set (Pickard et al.)
- `V2-P142` `ProductUse`: PROPr (PROMIS-Preference scoring system)
- `V2-P142` `ProductUse`: English EQ-5D-5L value set
- `V2-P142` `ProductUse`: Australian EQ-5D-5L value set (DCE with duration)
...
- `V2-P154` `ProductUse`: value sets for EQ-5D-3L, EQ-5D-5L, EQ-5D-Y, SF-6D, HUI, 15-D and AQoL identified via instrument websites
...
- `V2-P156` `ModelUse`: recommended EQ-5D-5L to EQ-5D-3L mapping function (van Hout et al. interim scoring / van Hout and Shaw algorithm)
...
- `V2-P180` `InstrumentUse`: EQ-5D-5L
- `V2-P180` `InstrumentUse`: EQ-5D-5L (self-reported impairment used for sampling strata)
...
- `V2-P188` `MethodUse`: Framework analysis [16, 17]
...
- `V2-P192` `ModelUse`: logistic regression model of incomplete EQ-5D response on age group, gender and long-term condition
