...
def registry_lookup(
    ...
    for row in rows:
        ...
        for label in labels:
            if label:
                lookup[(row["entity_type"], normalized_label(label))].add(registry_id)
    for row in alias_rows or []:
        ...
        lookup[(entity_type, normalized_label(row["alias"]))].add(registry_id)
...
def main() -> None:
    ...
    for row in read_tsv(args.manifest):
        ...
        source_runs[str(source_run)] += 1
        record = json.loads(source_path.read_text(encoding="utf-8"))
        for item in record["items"]:
            entity_type = REGISTRY_TYPE.get(item["type"])
            if not entity_type:
                continue
            key = (entity_type, normalized_label(item["source_label"]))
            matches = sorted(lookup.get(key, set()))
