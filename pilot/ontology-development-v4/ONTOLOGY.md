...
Status: version 0.13 production candidate after 60 development studies and two
20-publication calibration batches. It has not replaced the current graph.
...
The extraction agent cannot approve a new key, controlled value, or registry
identity. It proposes a new key or value through a `Gap`. For a missing
registry identity, it keeps the exact source label and leaves `registry_id`
empty. Deterministic validation sends that label to registry review.
...
The AI draft does not perform registry matching. Deterministic normalization
applies reviewed identities and aliases after extraction. The full registry is
not part of the AI prompt.
