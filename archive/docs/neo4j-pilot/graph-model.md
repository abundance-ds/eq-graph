...
Runnable DDL lives in [`graph/schema.cypher`](schema.cypher); this document explains *why* it is shaped that way.
...
- `Attribution` is PROV-O's qualified-relation pattern node for node. `prov:qualifiedAttribution` to `prov:Attribution` exists precisely to hang confidence and justification off an otherwise binary link.
