...
def main():
    ...
    with OPENALEX.open() as handle:
        for line in handle:
            records.append(normalize_record(json.loads(line), "openalex"))
    ...
    for profile in accepted:
        ...
        for row in payload.get("orcid_works") or []:
            records.append(normalize_record(row, "orcid", profile["name"]))
        for row in payload.get("pubmed_records") or []:
            records.append(normalize_record(row, "pubmed", profile["name"]))
