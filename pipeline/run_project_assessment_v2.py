#!/usr/bin/env python3
"""Run and aggregate the project-assessment v2 audit."""
...
def run_batch(output_folder, batch_id, resume):
    folder = output_folder / batch_id
...
def main():
    args = parse_args()
    output_folder = PILOT / f"project-assessment-{args.version}"
    selection = json.loads((output_folder / "selection.json").read_text())
    batch_ids = [batch["batch_id"] for batch in selection["batches"]]
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.workers) as pool:
        completed = list(pool.map(
            lambda batch_id: run_batch(output_folder, batch_id, args.resume), batch_ids
        ))
    rows = []
    ...
    fields = [
        "batch_id", "record_id", "project_link", "confidence", "project_ids",
        "project_evidence", "title",
    ]
    with (output_folder / "results.csv").open("w", newline="") as handle:
    ...
    summary = {
        ...
        "project_link": {
            value: sum(row["project_link"] == value for row in rows)
            for value in sorted({row["project_link"] for row in rows})
        },
