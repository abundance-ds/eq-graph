#!/usr/bin/env python3
"""Validated submission tool for a project-assessment v2 batch."""
...
def submit(args):
    record_id, link, confidence, project_ids, evidence = args
    data = manifest()
    ...
    selected = [] if project_ids == "none" else [
        value.strip() for value in project_ids.split(";") if value.strip()
    ]
    ...
    if not evidence.strip() or len(evidence) > 600:
    ...
    item = {
        ...
        "project_evidence": " ".join(evidence.split()),
    }
...
def main():
    ...
    if len(sys.argv) != 6:
        raise SystemExit(
            "usage: submit_project RECORD_ID LINK CONFIDENCE PROJECT_IDS EVIDENCE"
        )
    submit(sys.argv[1:])
